...
import { BRAND, getContextFromDeal, DEAL } from "../../util";

const getState = async (deal) => {
  const row = await db.DealModel.findOne({ deal });
...
const getAgentDetails = async (role_ids): Promise<any> => {
  const { rows } = await db.executeSql.promise(
    `SELECT
      deals_roles.id,
...
const isDoubleEnded = (deal) => {
  if (deal.context.ender_type !== undefined) {
...
const sync = async (deal) => {
  ...
  try {
    ...
    //   body,
    // });
    // if (res.successful) await save({ deal });
