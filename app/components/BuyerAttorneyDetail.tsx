...
const BuyerAttorneyDetail: React.FC<IStepProps> = ({
    updateStep,
    Components,
    models,
}) => {
    ...
    const buyerAttorneyRoles = roles.filter((role: IDealRole) => role.role === "BuyerPowerOfAttorney");

    console.log('buyerAttorneyRoles:', buyerAttorneyRoles);
                ...
                {(status === "Updating" || status === "Inserting") && (
                    <RoleForm
                        ...
                        form={status === "Inserting" ? (
                            { role: "BuyerPowerOfAttorney", ...getNameObject() }
                        ) : (
                            buyerAttorneyRoles[0]
                        )}
                    />
                )}
                {status === "Done" && (
                    <UserInfoCard
                        roleData={buyerAttorneyRoles[0]}
                        index={0}
                        step={4}
                        handleClickEditButton={handleClickEditButton}
                    />
...
export default BuyerAttorneyDetail;